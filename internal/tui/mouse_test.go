...
func TestQueueClickAndDoubleClick(t *testing.T) {
	...
	_, cmd := q.Update(click(2))
	if cmd == nil {
		t.Fatal("double click asked for nothing")
	}
	if got, ok := cmd().(openMsg); !ok || got.sel != (Selection{Repo: "acme/y", Number: 4}) {
		t.Errorf("double click asked to open %+v, want acme/y#4", got)
	}
}

func TestQueueClickBelowTheListUnderANoticeSelectsNothing(t *testing.T) {
	q := newQueue(t)
	q.notice = "could not open acme/x#8"
	for range 20 {
		q.items = append(q.items, q.items[0])
	}
	// The notice takes the list's last line, just above the footer.
	next, _ := q.Update(click(q.height - 2))
	if q = next.(QueueModel); q.cursor != 0 {
		t.Errorf("a click on the notice selected row %d", q.cursor)
	}
}
